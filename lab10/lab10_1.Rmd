...
```{r}
elephants%>%
  ggplot(aes(x=age, y=height))+geom_point(na.rm=T)+geom_smooth(method=lm, se=F)
```
...
homerange%>%
  ...
  geom_bar()+
  labs(title = "Summary of Thermal Regulation", 
       x="Type", 
       y="Count")+
...
```{r}
homerange%>%
  ggplot(aes(x=log10.mass, y=log10(mean.hra.m2), size=`mean.mass.g`))+
  geom_point()+
  labs(title = "Summary of Mass and Homerange", 
      x = "Mass(log10)", 
      y = "log10(Homerange)")+ 
  theme(plot.title = element_text(hjust = .5))
```
